...
public protocol CacheEngine {
    ...
    func get(product: String, in parentName: String, version: String, destination: Path) -> AnyPublisher<ArtifactType, Error>
...
public extension CacheEngine {
    ...
    func exists(artifact: AnyArtifact) -> AnyPublisher<Bool, Error> {
...
public struct AnyCacheEngine {
    ...
    private let _get: (String, String, String, Path) -> AnyPublisher<AnyArtifact, Error>
    ...
    public init<T: CacheEngine>(_ base: T) {
        ...
        _exists = base.exists
        _get = { base.get(product: $0, in: $1, version: $2, destination: $3)
    ...
    public func get(product: String, in parentName: String, version: String, destination: Path) -> AnyPublisher<AnyArtifact, Error> {
        return _get(product, parentName, version, destination)
    }
