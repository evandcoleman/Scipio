...
public enum ScipioError: LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .conflictingDependencies(let product, let dependencies):
            return "\(dependencies.count) dependencies (\(dependencies.joined(separator: ", "))) produce \(product). It is recommended to add \(product) as an explicit dependency and exclude it from the conflicting packages via the configuration file."
